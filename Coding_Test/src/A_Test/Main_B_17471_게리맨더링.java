package A_Test;

import java.util.*;
import java.io.*;

public class Main_B_17471_게리맨더링 {
	
	static int N,ans;
	static int t_sum,f_sum;
	static int [] population;
	static int [][] graph;
	static boolean [] V;
	static boolean [] D_V;
	static List<Integer> t_list,f_list;
	static Set<Integer> con;
	static boolean flag;

	public static void main(String[] args) throws Exception{
		System.setIn(new FileInputStream("input/17471.txt"));
		BufferedReader br = new BufferedReader(new InputStreamReader(System.in));
		StringTokenizer st;
		StringBuilder sb = new StringBuilder();
		
		N = Integer.parseInt(br.readLine()); // 정점 개수
		population = new int [N]; // 인구수 배열
		graph = new int [N+1][N+1]; // 인접행렬
		D_V = new boolean [N]; // 부분집합 방문 배열
		flag = false; // 연결상태 확인 변수
		ans = Integer.MAX_VALUE; // 답 담을 변수
		
		st = new StringTokenizer(br.readLine()); 
		
		for (int i = 0; i < N; i++) { 
			population[i] = Integer.parseInt(st.nextToken());
		}
		
		for (int i = 1; i <= N; i++) {
			st = new StringTokenizer(br.readLine()); // 인접행렬 생성
			int range = Integer.parseInt(st.nextToken());
			for (int j = 0; j < range; j++) {
				graph[i][Integer.parseInt(st.nextToken())] = 1;
			}
		}
		subset(0); 
		if (flag) {
			System.out.println(ans);
		}else {
			System.out.println(-1);
		}
	}

	private static void subset(int index) {
		if (index == N) {
			t_sum = 0; // true의 인구수 합
			f_sum = 0; // false의 인구수 합
			t_list = new ArrayList<Integer>(); // true에 속한 지역들
			f_list = new ArrayList<Integer>(); // false에 속한 지역들
			con = new HashSet<>(); // 정상연결인지 아닌지 판단하기 위해 만든 set
			
			// true에 해당하는 지역과 false에 해당하는 지역 add
			for (int i = 0; i < D_V.length; i++) {
				if (D_V[i] == true) t_list.add(i+1);
				if (D_V[i] == false) f_list.add(i+1);
			}
			
			// 한곳에 몰빵일때 종료
			if (t_list.size() == N || f_list.size() == N) return; 
			
			// true에 속한 지역들 dfs 실행 
			V = new boolean[N+1]; // 정점 방문 배열
			dfs(t_list.get(0));
			
			// false에 속한 지역들 dfs 실행 
			V = new boolean[N+1]; // 정점 방문 배열
			dfs(f_list.get(0));
			
			// 만약 con의크기가 정점의 개수와 동일하면 정상적으로 연결되있는 상태
			if (con.size()==N) {
				flag = true; // 현재 진행중인 부분집합이 정상 연결 상태
				
				// true와 false의 인구수 계산
				for (int i = 0; i < D_V.length; i++) {
					if(D_V[i] == true) t_sum += population[i];
					if(D_V[i] == false) f_sum += population[i];
				}
				
				ans = Math.min(ans, Math.abs(t_sum - f_sum));
			}
			
			return;
		}
		
		D_V[index] = true;
		subset(index+1);
		D_V[index] = false;
		subset(index+1);
	}

	private static void dfs(int index) {
		V[index] = true;
		// dfs 실행 -> dfs는 완탐이므로 지역구 하나당 한번씩 돌린결과 con SET에 저장
		con.add(index);
		
		// 지역구가 true일때
		if(D_V[index-1] == true) {
			for (int i = 1; i <= N; i++) {
				// 지역구가 true고 방문하지 않았고 인접행렬 1일때
				if (D_V[i-1] == true && V[i] == false && graph[index][i] == 1) {
					dfs(i);
				}
			}
		// 지역구가 false일때
		}else if (D_V[index-1] == false) {
			for (int i = 1; i <= N; i++) {
				// 지역구가 false고 방문하지 않았고 인접행렬 1일때
				if (D_V[i-1] == false && V[i] == false && graph[index][i] == 1) {
					dfs(i);
				}
			}
		}
	}
}
