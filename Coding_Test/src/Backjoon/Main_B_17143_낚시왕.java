...
public class Main_B_17143_낚시왕 {
    ...
    public static void main(String[] args) throws Exception {
        ...
        for (int i = 0; i < M; i++) {
			...
			map[x-1][y-1] = new Shark(x, y, speed, dir, size);
...
//        print(map);
        ...
        for (int i = 0; i < C; i++) {
//        	System.out.printf("-------%d번째 낚시  -------\n",i+1);
        	angler(i); // 낚시꾼이 상어 잡기
//        	print(map);
//        	System.out.printf("-------%d번째 move-------\n",i+1);
			move(); // 상어 이동
//			print(copy_map);
...
//						System.out.println(move);
						if (k==0) {
							if (d_x == 1 && move == 0) move = 1;
							if (d_x == R && move == 1) move = 0;
							if (d_y == C && move == 2) move = 3;
							if (d_y == 1 && move == 3) move = 2;
						}
						switch (move) {
						case 0: // 상
							d_x += dx[0];
...
//					System.out.printf("%d %d\n",d_x,d_y);
//					print(copy_map);
					if (copy_map[d_x-1][d_y-1] != null && copy_map[d_x-1][d_y-1].size > sz) {
						continue;
					}
					copy_map[d_x-1][d_y-1] = new Shark(d_x, d_y, range , move, sz);
//					System.out.println("--------------");
//					print(copy_map);
